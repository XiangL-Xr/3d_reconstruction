...
#import auto_rebuild.dataset.poses_utils.general as utils
from ..poses_utils import general as utils
...
def get_Ps(cameras,number_of_cameras):
    ...
    for i in range(0, number_of_cameras):
        ...
        intrinsic = cameras['camera_mat_%d' % i].astype(np.float64)
        Ps.append(P)
        intrs.append(intrinsic)
        #print('-- second intrinsic:', intrinsic)
    return np.array(Ps), np.array(intrs)
...
def get_all_mask_points(masks_dir):
    ...
    #print("length mask points:", len(mask_points_all))
    #print("length mask ims:", len(mask_ims))
    return mask_points_all,np.array(mask_ims)

def refine_visual_hull(masks, Ps, scale, center):
    num_cam=masks.shape[0]
    GRID_SIZE=100
    MINIMAL_VIEWS=15 # Fitted for DTU, might need to change for different data.
...
def get_normalization(source_dir, use_linear_init=False):
    ...
    masks_dir='{0}/masks'.format(source_dir)
    cameras=np.load('{0}/{1}_init.npz'.format(source_dir, cameras_filename))

    mask_points_all,masks_all=get_all_mask_points(masks_dir)
    number_of_cameras = len(masks_all)
    Ps, intrs = get_Ps(cameras, number_of_cameras)
    ...
    for i in range(number_of_cameras):
        cameras_new['camera_mat_%d' % i] = intrs[i]
