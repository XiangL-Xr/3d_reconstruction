...
class DatasetLLFF(Dataset):
    def __init__(self, base_dir, FLAGS, examples=None):
        ...
        poses        = poses_bounds.reshape([-1, 3, 5]).transpose([1,2,0])
        ...
        #print('self fovy: ', self.fovy)

        ######################## 去掉 recenter, modify by @lixiang #########################
        ...
        if self.FLAGS.local_rank == 0:
            print("Load DatasetLLFF: %d images with shape [%d, %d]" % (len(all_img), self.resolution[0], self.resolution[1]))
    ...
    def __getitem__(self, itr):
        ...
        print('img_alpha shape: ', img.shape)
        return {
            'mv' : mv,
            'mvp' : mvp,
            'campos' : campos,
